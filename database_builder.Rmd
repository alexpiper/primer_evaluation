...
pruned <- subset_tax(seqs, filtrank = "Class", filtvalue = "Insecta")
...
# problem is due to singletons - phytools ones is better
#has.singles(tree)
#test <- collapse.singles(tree, root.edge = FALSE)
...
tree2 <- tax2tree(nogaps, depth=5, output="phylo", replace_bads = TRUE)
...
# Get constraints for depth 6
tree3 <- tax2tree(nogaps, depth=6, output="phylo", replace_bads = TRUE)

# extract tree constraints from taxonomy
constraints <- castor::extract_fasttree_constraints(tree3)$constraints

# Write out constraints as fasta file
Ntips <- length(tree3$tip.label)
cat(paste(sapply(1:Ntips, #Ntips
    FUN=function(tip) sprintf(">%s\n%s\n",tree3$tip.label[tip],
    paste(as.character(constraints[tip,]),collapse=""))),collapse=""), file="reference/trees/constraints_depth6.fa")


# Write out an alignment with matching names
out <- nogaps
names(out) <- names(out) %>%
  str_remove("\\|.*$") %>%
  str_replace_all("\\,|\\;|\\:|\\(|\\)|\\[|\\]", "_")
...
FastTree -gtr -cat 20 -constraints constraints_depth5.fa -nt merged_final_nogaps.fa > constraint_depth5_FastTree.nwk
...
```{r}
tree1 <- read.tree("reference/trees/constraint_depth5_FastTree.nwk")
...
names <- names(seqs) %>%
  ...
  as_tibble() %>%
  tidyr::separate(V1, into=c("acc", "taxid"), sep="\\|") %>%
  mutate(acc = acc %>%stringr::str_replace_all("\\,|\\;|\\:|\\(|\\)|\\[|\\]", 
        "_"))
...
tree$tip.label <- tree$tip.label %>% 
  ...
  left_join(names) %>%
  mutate(spp = V8 %>% str_replace_all(" ", "_")) %>%
  pull(spp)


# Date usign congruify
ref_tree <- read.tree("reference/trees/chesters_2017_species_level_tree.nwk")
...
write.tree(res$phy, "reference/ultrametric_insecta_tree_depth5.nwk")

subtreeing = castor::get_subtree_with_tips(ref_tree, only_tips=tree$tip.label)
subtree = subtreeing$subtree

# map subset of target tips to reference tips
mapping = matrix(c(subtreeing$new2old_tip,(1:Ntip(subtree))), ncol=2, byrow=FALSE)
