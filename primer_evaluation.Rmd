...
folmer_id_spp <- vroom::vroom("primer_evaluation/amplicons/usearch_output/LCO1490_HCO2198.txt.gz",
    ...
    dplyr::summarise(mixed=n_distinct(spp2)) %>%
    ungroup() %>%
  mutate(species = spp1 %>% str_replace_all(" ", "_")) %>%
  select(threshold, species, values = mixed)
...
for(i in 1:length(thresholds)){
  ...
  consentrait <- get_trait_depth(tree, tip_states_binary, count_singletons=FALSE, weighted=TRUE, Npermutations = 1000)
...
ggplot(acf, aes(x = distances, y=value)) +
  geom_line() +
  facet_grid(type~threshold, scales="free_y")
...
imputed <- genus_summaries %>%
  ...
  left_join(primers %>%
            dplyr::select(primer = name, degeneracy))%>%
  filter(!is.na(hsp))
...
p3 <- p3 + geom_cladelabel(node=cls[[i]], label=names(cls[i]), align=T, angle=270, hjust='center', offset.text=.25, barsize=1.5, color = colour.pal[[i]])
}
...
gg.primer_stats <- imputed %>%
  ...
  ggplot(aes(x = primer, y = value, fill=value)) +
    geom_col() +
    scale_fill_gradient(low = "darkslateblue", high = "firebrick", na.value = "grey", oob = scales::squish) +
    facet_grid(measure~strand, scales="free", drop=TRUE) +
  base_theme+
    theme(
      #axis.text.y = element_blank(),
      #axis.ticks.y = element_blank(),
...
Fig5a <- p3 + gg.mismatch + gg.density + plot_layout(widths = c(1,3,0.25))
Fig5b <- plot_spacer() + gg.primer_stats + plot_spacer() + plot_layout(widths = c(1,3,0.25))
...
for (i in 1:length(uprimers)){
  ...
  # Autocorrelation funciton
  acf <- castor::get_trait_acf(subset_tree, tip_states, Npairs=1e9, Nbins=100)
...
# Mean Autocorrelation across all primers
tree <- ape::read.tree("reference/trees/ultrametric_insecta_tree_order_constrained.nwk")
...
# Autocorrelation funciton
acf_mean <- castor::get_trait_acf(subset_tree, tip_states, Npairs=1e8, Nbins=100)
  
# Create output data frame
data.frame(
    distances = acf_mean$distances,
    acf_acf = acf_mean$autocorrelations) %>%
ggplot(aes(x = distances, y=acf_acf)) +
  geom_line() +
  base_theme
