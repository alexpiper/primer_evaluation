...
ent <- taxreturn::alignment_entropy(seqs, maskgaps=1, countgaps=FALSE, method="ML", unit="log")
...
# Get number of sequences at each positiin

```{r bases per position }
seqs <- insect::readFASTA("reference/merged_final.fa.gz")

seqs <- as.character(seqs)
seqs <- lapply(seqs, toupper)

MSA <- matrix(as.vector(unlist(seqs)), ncol = length(seqs[[1]]), byrow = TRUE)
n_pos <- length(MSA[1, ])
n_seq <- length(MSA[, 1])
...
for (i in 1:n_pos) {
  pos_count <- table(MSA[, i])
  tab[[i]]  <- sum(pos_count[names(pos_count) %in% names])
}

out <- unlist(tab)
names(out) <- seq(1,n_pos,1)
...
# Arrange on tree
library(castor)
tree <- ape::read.tree("reference/ultrametric_insecta_tree.nwk")
