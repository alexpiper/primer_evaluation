...
Fig3 <- gg.entropy / gg.primers / gg.sequences + plot_layout(heights= c(3, 3, 0.5))  +  plot_annotation(tag_levels = 'A') 
...
ent_sw <- ent %>%
  ...
  mutate(sw100 = sw(ent, width=100, interval = 1)/100) %>%
  mutate(sw200 = sw(ent, width=200, interval = 1)/200) %>%
  mutate(sw300 = sw(ent, width=300, interval = 1)/300) %>%
  mutate(sw400 = sw(ent, width=400, interval = 1)/400) %>%
...
gg.swent <-  ent_sw %>%
 ...
 #  )) %>%
  ...
  facet_wrap(windowsize~., ncol=1, strip.position ="right") +
  scale_x_continuous(limits = c(0, 712), breaks=seq(0,700,50), expand=c(0,0))  +
  theme_void() +
  theme(legend.position = "bottom",
...
Fig3 <- gg.entropy / gg.primers / gg.sequences / gg.swline + plot_layout(heights= c(3, 3, 0.5, 2)) 
...
# Make 100bp windows
dir.create("primer_evaluation/amplicons/win100/")
alignment_sw(seqs, width=100, interval=3, maxgaps=9) %>%
  purrr::map2(., names(.), ~Biostrings::writeXStringSet(.x, file=paste0("primer_evaluation/amplicons/win100/", .y,".fa.gz"), compress=TRUE))
...
sw_clusters <- c(fs::dir_ls(path="primer_evaluation/amplicons/win100/usearch_output/", glob = "*.txt.gz"),
                    fs::dir_ls(path="primer_evaluation/amplicons/win200/usearch_output/", glob = "*.txt.gz"),
...
#Get amplified
amplified <-c(fs::dir_ls(path="primer_evaluation/amplicons/win100/", glob = "*.fa.gz"),
                    fs::dir_ls(path="primer_evaluation/amplicons/win200/", glob = "*.fa.gz"),
...
gg.id <- id_success %>%
  ...
  geom_text(data=break_annot, aes(x=amplicon, y=preds,
                                  label=paste0(breakpoint, "bp p",ifelse(pval < 0.001,"<0.001", ">0.001"))),
...
folmer_id <- vroom::vroom("primer_evaluation/amplicons/usearch_output/LCO1490_HCO2198.txt.gz",
    ...
    ungroup() %>%
  mutate(spp1 = spp1 %>% str_replace_all(" ", "_")) %>%
  mutate(Genus = spp1 %>% str_remove("_.*$")) %>%
  group_by(Genus, threshold) %>%
  dplyr::summarise(mixed=sum(mixed)) 
...
folmer_id_filtered <- folmer_id %>%
  dplyr::filter(Genus %in%  genus_tree$tip.label)
...
for(i in 1:length(thresholds)){
  ...
  weights_p1 <- p1$data %>%
    left_join(folmer_id_filtered %>% 
                ...
                select(-threshold) %>%
                dplyr::rename(label = Genus, values = mixed))%>%
  ...
  p2 <- p1 %<+% weights_p1 +
    ...
    #geom_tippoint(aes(colour=highlight), size=.5) +
    theme(legend.position = "none") + 
  ...
  for(l in 1:length(cls)){
    p3 <- p3 + geom_cladelabel(
      ...
      align=T, angle=270, hjust='center', offset.text=.25, 
...
gg.bad_clades <- bad_clades %>% 
  ggplot(aes(x= mean_depth_pos_clades, y=traits_per_pos_clades)) + 
...
Fig4 <- Fig4a - gg.id - (gg.acf + gg.bad_clades)

Fig4

pdf(file="fig/Fig4_id_success.pdf", width = 8, height = 11 , paper="a4")
  plot(Fig4)
try(dev.off(), silent=TRUE)

  
test1 <-  (gg.acf + gg.bad_clades)

gg.id - test1 + plot_layout(widths=c(2,1))
  
#  Look at clade with the highest positives
hist(consentrait$positives_per_clade)

pos <- consentrait$positives_per_clade

which.max(pos)

subset_tree <- get_subtree_at_node(tree, which.max(pos) -Ntip(tree))$subtree
subset_tree$tip.label
...
```
# Could put the autocorrelation function below the tree on the left
...
for (i in 1:length(uprimers)){
  ...
  #tip_states_binary <- tip_states
  #tip_states_binary[tip_states_binary >0] <- 1
  #consentrait <- get_trait_depth(subset_tree, tip_states_binary, count_singletons=FALSE, weighted=TRUE, Npermutations = 1000)
  ...
  acfvec[[i]] <- data.frame(
    ...
    acf_npairs = acf$Npairs_per_distance
    #,
    #trait_mean = consentrait$mean_depth,
    #trait_std = sqrt(consentrait$var_depth),
    #trait_n = consentrait$Npositives,
    #trait_p = consentrait$P
    )
}
...
gg.all_acf <- acf %>%
  ...
  pivot_longer(starts_with("acf_"),
               ...
               values_to="value") %>%
 filter(type == "acf_acf") %>%
ggplot(aes(x = distances, y=value, group= primer)) +
...
pdf(file="fig/supplementary/all_acf.pdf", width = 8, height = 11 , paper="a4")
  plot(gg.all_acf)
try(dev.off(), silent=TRUE)
```
