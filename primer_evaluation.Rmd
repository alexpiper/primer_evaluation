...
# Arrange on tree
tree <- ape::read.tree("reference/trees/ultrametric_insecta_tree_fulltree.nwk")
...
p1 <- ggtree(tree2, ladderize=TRUE, aes(colour=values))# + #,layout = "circular"
...
test <- tax_groups %>%
  ungroup() %>%
  dplyr::filter(Order == "Zygentoma") %>%
  mutate(genus.y = label, genus.x = label) %>%
  tidyr::expand(genus.y, genus.x) %>%
  dplyr::filter(!genus.x == genus.y)
test2 <- get_pairwise_mrcas(tree = tree, A = test$genus.x, B=test$genus.y)
mode_node <- tail(names(sort(table(test2))), 5)

p3 <- p2
p3 <- p3 + geom_cladelabel(node=mode_node[1], label="Lepidoptera", align=T, angle=270, hjust='center', offset.text=.5, barsize=1.5)
p3
...
# Arrange on tree
library(castor)
tree <- rncl::read_newick_phylo("reference/trees/ultrametric_insecta_tree_depth5.nwk")
