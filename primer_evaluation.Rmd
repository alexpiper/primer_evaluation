...
gg.id <- id_success %>%
  ...
  geom_line(data=dat_pred[dat_pred$amplicon >= breakpoint,],
            aes(x=amplicon, y=preds),  color = 'red', inherit.aes = FALSE) +
  geom_point(data= break_annot, aes(x=amplicon, y=preds),  color = 'red', inherit.aes = FALSE) +
  geom_text(data= break_annot, aes(x=amplicon, y=preds, label=paste0(breakpoint, "bp")),nudge_y = 0.1,  color = 'red', inherit.aes = FALSE) +
...
tips_to_keep <- tree$tip.label %>% 
  ...
  #filter(Genus %in% (folmer_id %>% pull(Genus))) %>%
  group_by(Genus) %>%
  dplyr::slice(1) %>%
  ungroup() %>%
...
thresholds <- unique(folmer_id$threshold)
...
for(i in 1:length(thresholds)){
  ...
  p1 <- ggtree(tree , ladderize=TRUE, aes(colour=values), layout = "circular") 
...
#
  ...
  cls <- tax_groups %>% 
    ...
    purrr::map(function(x){
      ...
      if(length(quants$label) > 1){
        mrca_node <- get_mrca_of_set(tree, quants$label)
        ...
        subset_tree <- get_subtree_at_node(tree, subset_node)$subtree
  ...
  # Calculate autocorrelation 
	...
	acfvec[[i]] <- castor::get_trait_acf(tree, tip_states, Npairs=1e8, Nbins=100)
  
}
plot(acf_results$distances, acf_results$autocorrelations, type="l", xlab="distance", ylab="ACF")

# ACF might not make sense for this, but it probably does for the primers
...
summaries <- vroom::vroom("primer_evaluation/mismatch_summary.csv.gz", delim = ",") %>%
  dplyr::select(-dir) %>%
  mutate(primer = primer %>% str_remove(".gz$")) %>%
  left_join(primers %>%
...
tree <- castor::get_subtree_with_tips(tree,
                                          only_tips = tree$tip.label[tree$tip.label %in% summaries$Genus],
                                          collapse_monofurcations=TRUE,
                                          force_keep_root=TRUE)$subtree
...
#  filter(Genus == "Machilis") %>%
#  pull(tips)
#tree <- root_via_outgroup(tree, outgroup="Machilis", update_indices = TRUE)
...
p1 <- ggtree(tree , ladderize=TRUE, aes(colour=values))
...
Fig4 <- p3 + gg.mismatch + gg.density + plot_layout(widths = c(1,3,0.25))

pdf(file="fig/Fig4_mismatch.pdf", width = 8, height = 11 , paper="a4")
  plot(Fig4)
try(dev.off(), silent=TRUE)
