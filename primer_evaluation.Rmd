...
#vroom::vroom_write(joint, "primer_evaluation/amplicons/mixed_clusters_summary2.csv", delim=",")


# Make figure

# Count reads
joint <- vroom::vroom("primer_evaluation/amplicons/mixed_clusters_summary2.csv", delim=",") %>%
  left_join(combos) %>% 
  mutate(success =  (spp_amplified - mixed)/spp_amplified) %>%
...
model_dat <- id_success %>%
  dplyr::select(combos, amplicon, threshold, success, success_alr, success_rat)
...
breakpoint_fit <- model_dat %>% 
  ...
  mutate(fits = map(data, ~chngptm(formula.1=success~1, formula.2=~amplicon,
         ...
         aug = map2(fits, data, function(x,y){
           preds <- predict(x,y)
           out <- y
           out$preds <- preds
           out$res <- NA
...
gg.id <- id_success %>%
  ...
  #scale_colour_gradient2(low ="firebrick",mid="goldenrod", high ="darkslateblue", na.value = "grey") +
...
gg.failed <- folmer_id_spp %>%
  ...
  left_join(lineage) %>%
  group_by(Order, Genus, threshold) %>%
  ...
  top_n(10, values) %>%
    ...
    Genus  = factor(Genus ),
    Genus  = reorder_within(Genus, values, threshold)) %>%
  ggplot(aes(x=Genus, y=values, fill=Order))+
  geom_col() +
  scale_fill_brewer(palette = "Paired")+
...
gg.bad_clades <- bad_clades %>% 
  ...
  ggplot(aes(x= mean_depth_pos_clades, y=traits_per_pos_clades, fill=traits_per_pos_clades)) + 
  geom_jitter(alpha=0.2, shape=21, height=.2, width=.2) + 
  geom_point(data = bad_clades_labels, aes(x=mean_depth_pos_clades, y=traits_per_pos_clades), colour="firebrick", alpha=0.8)+
  geom_text_repel(data = bad_clades_labels, aes(x=mean_depth_pos_clades, y=traits_per_pos_clades, label= subtree_taxonomy, colour=order_taxonomy))+
  scale_fill_viridis_c(option="C", trans="log10")+
  #scale_fill_gradient(low ="darkslateblue", high ="firebrick", na.value = "grey") +
  scale_color_brewer(palette = "Paired")+
    facet_grid(~threshold) +
  scale_y_log10() +
  base_theme+
...
Fig4 <-  gg.id /  gg.bad_clades / gg.acf + plot_layout(heights = c(4,2,1)) + plot_annotation(tag_levels = "A")
...
model_df <- imputed %>% 
  left_join(primers) %>%
  select(primer, values, hsp, start, end, degeneracy)%>%
  pivot_longer(cols=c("values", "hsp"),
               names_to="type",
               values_to = "values")#%>%
...
primer_stats <- primers %>%
  ...
  select(primer = name, strand,GC = `GC%`, GC_last2, tm, degeneracy, length, poly, nseqs = seqs) %>%
  #mutate(degeneracy = log10(degeneracy)) %>%
  pivot_longer(3:ncol(.),
               names_to = "metric",
               values_to ="value") %>%
    ...
    metric == "GC_last2" & value == 2 ~ "good",
    metric == "GC_last2" & value == 1 ~ "medium",
    metric == "GC_last2" & value == 0 ~ "bad",
    ...
    metric == "poly" & value <= 3 ~ "good",
    metric == "poly" & value <= 4 ~ "medium",
    metric == "poly" & value > 4~ "bad",
    
    metric == "tm" & between(value, 50, 60) ~ "good",
    metric == "tm" & (between(value, 45, 50) | between(value, 60, 65)) ~ "medium",
    metric == "tm" & (value < 45 | value > 65) ~ "bad",
...
mismatch_summary <-  vroom::vroom("primer_evaluation/mismatch_summary.csv.gz",delim = ",") %>%
  ...
  group_by(primer, strand) %>%
  summarise(mismatch = mean(sum, na.rm=TRUE),sd = sd(sum)) %>%
  mutate(upper = mismatch + 2*sd, 
         lower  = mismatch -2*sd) %>%
...
# Id success summary
id_success_summary <- vroom::vroom("primer_evaluation/amplicons/mixed_clusters_summary.csv", delim=",") %>%
  mutate(success =  (spp_amplified - mixed)/spp_amplified,
         threshold = 1-dist) %>%
  ...
  select(primer, strand, threshold, success, type)%>%
  filter(threshold  >= 0.97) %>%
  group_by(primer, strand, type) %>%
  summarise(success = mean(success, na.rm=TRUE), sd = sd(success, na.rm = TRUE)) %>%
  mutate(upper = success + 2*sd, 
         lower  = success -2*sd)%>%
  mutate(label = case_when(
    success > 0.85~ "good",
    between(success, 0.80, 0.85) ~ "medium",
    success < 0.80 ~ "bad"
  ))
...
gg.primer_stats <-primer_stats %>%
  ...
  mutate(metric = factor(metric, levels = c("nseqs","degeneracy","length", "tm", "GC", "GC_last2", "poly"))) %>%
  left_join(ranks)%>%
  mutate(primer = factor(primer)) %>%
  ggplot(aes(x = fct_reorder(primer, -rank), y = metric, fill=label)) + 
  geom_tile(alpha=1) +
  geom_text(aes(label=value), colour="black") +
  facet_grid(strand~., drop = TRUE, scales = "free") +
  base_theme + 
  scale_fill_manual(values = c(bad ="#fc8d59", medium="#ffffbf", good="#91bfdb")) +
  coord_flip() +
...
gg.mean_mismatch <- mismatch_summary %>% 
  ...
  geom_pointrange(aes(ymin=lower, ymax = upper, colour=label)) +
  #geom_boxplot(outlier.alpha = 0.1) +
  #scale_y_log10()+
  facet_grid(strand~., drop = TRUE, scales = "free") +
  base_theme + 
  scale_colour_manual(values = c(bad ="#fc8d59", medium="#ffffbf", good="#91bfdb")) +
  coord_flip() +
...
gg.primer_success <- id_success_summary  %>%
    left_join(ranks)%>%
  mutate(primer = factor(primer))%>%
  ggplot(aes(x = fct_reorder(primer, -rank), y = success, colour=label)) +
  geom_pointrange(aes(ymin=lower, ymax = upper)) +
  #geom_boxplot(outlier.alpha = 0.1) +
  #scale_y_log10()+
  facet_grid(strand~type, drop = TRUE, scales = "free") +
  base_theme + 
  scale_colour_manual(values = c(bad ="#fc8d59", medium="#ffffbf", good="#91bfdb")) +
...
# Plotting
gg.primer_success + gg.mean_mismatch + gg.primer_stats + plot_layout(widths =c(2,1, 6))

# Add mean ID success for windows starting at end base of forward, or start base of reverse



# OLD SW Id success plot
...
primers %>%
  select(name, strand, start, end) %>%
  left_join(sw_means %>%
            filter(threshold > 0.97) %>%
            group_by(win_length, pos) %>% 
            summarise(success = mean(success, na.rm=TRUE)) %>%
            select(start = pos, start_success = success, win_length)) %>%
  left_join(sw_means %>%
             filter(threshold > 0.97) %>%
            group_by(win_length, pos) %>% 
            summarise(success = mean (success)) %>%
            select(end = pos, end_success = success, win_length)) %>%
  mutate(id_success = case_when(
    strand == "F" ~ end_success,
    strand == "R" ~ start_success
  )) %>%
  select(primer = name, strand, id_success, win_length)%>% 
  ggplot(aes(x = primer, y = id_success, colour=win_length)) +
  geom_point() +
  #geom_boxplot(outlier.alpha = 0.1) +
  #scale_y_log10()+
  facet_grid(strand~., drop = TRUE, scales = "free") +
