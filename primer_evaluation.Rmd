...
gg.swid <- sw_means %>%
    ...
    ggplot(aes(x = pos, y=1)) +
      ...
      scale_fill_viridis_c(option="plasma") + 
    facet_wrap(win_length ~., ncol=1, strip.position ="right") +
    scale_x_continuous(limits = c(0, 712), breaks=seq(0,700,50), expand=c(0,0))  +
    #theme_void() +
    theme(legend.position = "bottom",
          axis.text.y = element_blank(),
...
ref_pos <-  c(0.97, 0.98,0.99,1) %>%
      purrr::map(~{
        data.frame(pos = seq(1, 712, 1)) %>%
          mutate(threshold = .x)
      }) %>%
      bind_rows()

gg.swid <-  sw_joint %>%
  dplyr::select(win_length, pos, success, threshold) %>%
  group_by(win_length) %>%
  group_split() %>%
  purrr::map(~{
    .x %>%
      right_join(ref_pos %>% filter(pos < max(.x$pos))) %>%
      mutate(win_length = unique(.x$win_length))
  }) %>%
  bind_rows() %>%
  ungroup()%>%
  group_by(win_length, threshold ) %>%
  arrange(pos) %>%
  tidyr::fill(success, .direction = "down")%>%
  mutate(success_imp = ma(success, 3)) %>%
  filter(!is.na(success_imp)) %>%
  filter(threshold == 0.97) %>%
    ggplot(aes(x = pos, y=1)) +
      geom_tile(aes(fill=success_imp))+
      scale_fill_viridis_c(option="plasma") + 
    facet_wrap(win_length ~., ncol=1, strip.position ="right") +
    scale_x_continuous(limits = c(0, 712), breaks=seq(0,700,50), expand=c(0,0))  +
    theme_void() +
...
gg.swline <-  sw_joint %>%
  ...
  filter(!is.na(success_imp)) %>%
  filter(threshold == 0.97) %>%
  ggplot(aes(x = pos, y=success_imp)) +
  geom_line()+
  facet_wrap(win_length~., ncol=1, strip.position ="right") +
  scale_x_continuous(limits = c(0, 712), breaks=seq(0,700,50), expand=c(0,0))  +
  theme_void() +
...
ggplot(aes(x = distances, y=value)) +
  ...
  theme_void() +
  facet_grid(primer~strand, drop = TRUE, scales="free_x") 
